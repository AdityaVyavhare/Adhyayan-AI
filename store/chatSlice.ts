  ...
  reducers: {
    setActiveTool: (
      ...
      action: PayloadAction<
        "general" | "pdf" | "youtube" | "scan" | "podcast" | "gita"
      >,
    ) => {
    ...
    setCategoryChats: (
      state,
      action: PayloadAction<{
        tool: "general" | "pdf" | "youtube" | "scan" | "podcast" | "gita";
        chats: any[];
      }>,
    ) => {
      ...
      const formattedChats = chats.map((c) => {
        if (typeof c === "string")
          return { id: c, title: "Chat", date: new Date().toISOString() };
