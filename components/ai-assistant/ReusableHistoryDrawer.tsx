...
import { setCategoryChats, setCurrentChat, setLoadingConversation, startNewChat } from '@/store/chatSlice';
...
interface ReusableHistoryDrawerProps {
  ...
  onClose?: () => void;
  isModal?: boolean; 
  toolType?: 'pdf' | 'chat' | 'youtube' | 'scan' | 'all' | 'podcast'; // New Prop for filtering
}
...
export default function ReusableHistoryDrawer({ 
  ...
  const { user } = useUser();
  const { chatsByTool, chatIds, currentChatId, loadingConversation } = useSelector((state: RootState) => state.chat);
  ...
  const filteredChatIds = React.useMemo(() => {
      ...
      const allSessions = [...chatsByTool.general, ...chatsByTool.pdf, ...chatsByTool.youtube, ...chatsByTool.scan];
  ...
  const handleSelectChat = async (id: string) => {
    ...
    try {
        ...
        if (toolType === 'youtube') {
            messages = await ChatService.fetchVideoChatHistory(user.primaryEmailAddress.emailAddress, id);
        } else if (toolType === 'pdf') {
            messages = await ChatService.fetchPdfConversation(user.primaryEmailAddress.emailAddress, id, threadId);
        } else if (toolType === 'scan') {
            const result = await ChatService.fetchScanConversation(user.primaryEmailAddress.emailAddress, id);
            ...
            messages = await ChatService.fetchChatModelConversation(user.primaryEmailAddress.emailAddress, id, threadId);
        }
  ...
  const handleDeleteChat = async (chatId: string) => {
      ...
      const performDelete = async () => {
          let success = false;
          if (toolType === 'youtube') {
              success = await ChatService.deleteVideoChat(user.primaryEmailAddress!.emailAddress, chatId);
          } else if (toolType === 'scan') {
              success = await ChatService.deleteScanChat(user.primaryEmailAddress!.emailAddress, chatId);
          } else if (toolType === 'pdf') {
              success = await ChatService.deletePdfChat(user.primaryEmailAddress!.emailAddress, chatId);
          } else if (toolType === 'podcast') {
              success = await ChatService.deletePodcastChat(user.primaryEmailAddress!.emailAddress, chatId);
          } else {
              success = await ChatService.deleteChat(user.primaryEmailAddress!.emailAddress, chatId);
          }
          ...
          if (success) {
              ...
              if (toolType === 'all' || toolType === 'chat') {
                  ...
                  if (updated.length !== chatsByTool.general.length) dispatch(setCategoryChats({ tool: 'general', chats: updated }));
              }
              if (toolType === 'all' || toolType === 'pdf') {
                  const updated = remove(chatsByTool.pdf);
                  if (updated.length !== chatsByTool.pdf.length) dispatch(setCategoryChats({ tool: 'pdf', chats: updated }));
              }
              if (toolType === 'all' || toolType === 'youtube') {
                  const updated = remove(chatsByTool.youtube);
                  if (updated.length !== chatsByTool.youtube.length) dispatch(setCategoryChats({ tool: 'youtube', chats: updated }));
              }
              if (toolType === 'all' || toolType === 'scan') {
                  const updated = remove(chatsByTool.scan);
                  if (updated.length !== chatsByTool.scan.length) dispatch(setCategoryChats({ tool: 'scan', chats: updated }));
              }
              if (toolType === 'all' || toolType === 'podcast') {
                  const list = (chatsByTool as any).podcast || [];
                  const updated = remove(list);
                  // We need to ensure setCategoryChats supports 'podcast' or dispatch generic
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {filteredChatIds.length === 0 ? (
            <Text style={{padding: 16, color: '#666', fontStyle:'italic'}}>No recent chats found.</Text>
        ) : (
             filteredChatIds.map((chat: any) => (
                 <DrawerItem 
                    ...
                    subtitle={chat.date ? new Date(chat.date).toLocaleDateString() : ""} 
      ...
      <View style={styles.footerContainer}>
           <TouchableOpacity style={styles.startNewChatButton} onPress={handleNewChat}>
...
function DrawerItem({ chatId, title, subtitle, onPress, onDelete, isActive }: any) {
    return (
      <TouchableOpacity 
      ...
      >
         <View style={styles.cardHeader}>
            <Text style={[styles.cardTitle, isActive && styles.activeText]} numberOfLines={1}>{title}</Text>
         </View>
         <Text style={[styles.cardSubtitle, isActive && styles.activeSubtitle]} numberOfLines={1}>{subtitle}</Text>
         <View style={styles.cardFooter}>
             <Text style={[styles.cardDate, isActive && styles.activeSubtitle]}>10:42 AM</Text>
             {onDelete && (
                 <TouchableOpacity style={styles.deleteButton} onPress={onDelete}>
                     <Ionicons name="trash-outline" size={16} color={isActive ? "#ef4444" : "#ff6b6b"} />
                 </TouchableOpacity>
             )}
         </View>
      </TouchableOpacity>
...
function UserProfileFooter() {
    ...
    const displayUser = user || {
        ...
        email: clerkUser?.primaryEmailAddress?.emailAddress || 'student@example.com',
    ...
    return (
        <View style={styles.userFooter}>
            {displayUser.avatarUrl ? (
                 <Image source={{ uri: displayUser.avatarUrl }} style={styles.userAvatar} />
            ) : (
                <View style={[styles.userAvatar, {justifyContent: 'center', alignItems: 'center'}]}>
