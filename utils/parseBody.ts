...
/**
 * Parse an educational AI body string into ordered paragraphs and segments.
 *
 ...
 *
 * Parsing rules:
 * - Split body by "*/*" into paragraphs.
 * - For each paragraph, scan left-to-right and emit segments in order:
 *   text, code, formula.
 * - Markers are removed from output.
 * - Escaping:
 *   - Code: convert "\\n" → "\n"
 *   - Formula: convert "\\\\" → "\"
 */
export function parseBody(body: string): ParsedParagraph[] {
  ...
  for (const rawParagraph of paragraphsRaw) {
    ...
    while (index < paragraph.length) {
      ...
      if (!kind || pos === -1) {
        // No more markers: remaining is plain text.
      ...
      if (kind === 'code') {
        ...
        if (endIndex === -1) {
          // Malformed: treat marker as plain text and stop special parsing.
